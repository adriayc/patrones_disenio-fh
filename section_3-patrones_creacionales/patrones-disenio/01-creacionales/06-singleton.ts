...
class DragonBalls {
  ...
  collectBall(): void {
    if (this.ballsCollected < 7) {
      this.ballsCollected++;
      console.log(
        `Esferas de Dragón recolectada. Total de esferas: ${this.ballsCollected}`
      );
      return;
    }
  ...
  summonShenlong() {
    ...
    console.log(
      ...
      } esfera(s) de Drágon para invocar a Shenlong`
    );
  }
}

function main() {
  //   const gokuDragonBalls = new DragonBalls(); // Error (constructor is private)
